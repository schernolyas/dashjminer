...
public class ByteUtils {
    ...
    public static byte[] prepareForX11Hash(byte[] blockHeaderCandidate) throws IOException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream(80);
        byte[] portion = new byte[4];
        for (int position = 0; position < 80; position += 4) {            
            System.arraycopy(blockHeaderCandidate, position, portion, 0, 4);
            baos.write(bigEndian(portion));
        }
        baos.flush();
        return baos.toByteArray();
    }
